...
import { tonweb, TonWeb, version, wc } from './_utils.js';
...
export async function prepareCollection(collectionConfig) {
  ...
  const { publicKey, secretKey } = await tonMnemonic.mnemonicToKeyPair(words);
  ...
  collectionConfig._wallet = new WalletClass(tonweb.provider, {
    publicKey,
    wc,
  });
  ...
  collectionConfig._royaltyAddress = new TonWeb.utils.Address(
    collectionConfig.collectionData.royaltyAddress,
  );
